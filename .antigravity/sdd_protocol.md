...
4. **Spotless Formatting Gate**: Always run `./mvnw spotless:check` (or auto-format with `./mvnw spotless:apply`) before completing any wave. Spotless is enforced by the Maven verify/package lifecycle and CI; unformatted Java files will fail `./redeploy.sh`.
5. **Reviewer Audit**: Invoke `spring_reviewer` (with `Workspace: inherit`) to audit the uncommitted changes for security, N+1 queries, transactional integrity, and Spotless compliance.
6. **Remediation**: If the reviewer finds material issues, present them to the user. Upon approval, invoke `spring_implementer` to remediate, followed by another reviewer check. When a fix lands, ask what parameter, branch or overload can now be **deleted** so the mistake cannot be re-expressed — an unrepresentable bug outlasts any rule written down about it.
