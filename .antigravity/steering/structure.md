...
- **`V9__orchestrator_profile_prototype_flag.sql`** adds `orchestrator_profiles.prototype BOOLEAN NOT NULL DEFAULT FALSE`, mirroring what `V8__playbook_prototype_flag.sql` did for `playbooks`. It is a **discovery** flag and nothing else: `ResolvedProfile` deliberately does not carry it, so a run resolves and executes identically whether or not the profile is experimental, and both chat clients hide it under the one setting the user already controls for prototype playbooks (`ConversationSetting.SHOW_PROTOTYPES` on the web, `AppSettings.showPrototypePlaybooks` on the desktop). Three consequences the tests pin. The server sends every profile **flagged, never filtered** (`ChatController` via `OrchestratorProfileService.listProfileOptions`, `DesktopSyncController` via `OrchestratorProfileDto`) — filtering server-side would make a client-side toggle need a page reload, and would drop the conversation's own selected profile out of its dropdown. The profile a conversation has already selected is therefore **always** listed, prototype or not, or the selector reads "Single playbook" over a conversation still running in multi-agent mode. And unlike this table's other column defaults (see `tech.md`), this `DEFAULT FALSE` is **reachable and load-bearing**: `yvoke-exports`' `_profiles_upsert` names the column now, but a restore or a hand-run INSERT does not, and a NULL there would be invisible to both `WHERE prototype` and `WHERE NOT prototype`.
