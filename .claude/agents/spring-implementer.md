...
- **Authorize + validate** — gate every endpoint with `@PreAuthorize`/the filter chain (deny by default), enforce caller ownership of the data touched, and validate external input with `@Valid` + Bean Validation.
...
3. **Verify**: Confirm the Docker daemon is running, then run the narrowest compilation/test check first before broadening. When touching packaging, layering, or migrations, also run `ArchitectureTest` and `python3 .antigravity/scripts/check_steering.py`. Use `@MockitoBean` (not the deprecated `@MockBean`) in Spring Boot 4 tests.
